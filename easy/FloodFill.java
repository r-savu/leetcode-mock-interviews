...
class Solution {
    ...
    private void fill(int sr, int sc, int oldColor, int newColor) {
    ...
    public int[][] floodFill(int[][] image, int sr, int sc, int newColor) {
        ...
        img = new int[image.length][];
        m = image.length;
        n = image[0].length;
        for (int i = 0; i < m; i++) {
            img[i] = new int[image[0].length];
            for (int j = 0; j < n; j++) {
                img[i][j] = image[i][j];
            }
        }

        fill(sr, sc, oldColor, newColor);
        return img;
    }
}
